...
- **Past `patch_envelope.MAX_FILES` (40) targets in one call**, the remaining files are not parsed and so are not judged. The cap is a latency guard, not a verdict: a call that changes more files than that is not refused, it is judged on the first 40 and unknown on the rest.
- **An envelope the call was never going to write.** The parse keys on the envelope markers in the input text, not on what the tool means to do, because the tool name is unreliable here (this host's Codex writes through `exec`, seen as `Bash`). So a command that merely *carries* envelope text — `grep` printing a patch, `cat` of a `.patch` file, a here-doc quoting one as an example — is read as if it were the write. Added lines that match a ruling are blocked in that case too. This is a deliberate trade: the alternative is to guess intent from the command, and a guess that lets a real write through is the failure this gate exists to remove. Quoting the pattern as prose (inside quotes, not on a `+` line) is not affected; 2026-09-22 verification exercised both shapes.

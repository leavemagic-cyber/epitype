...
產生的資料庫位於 `<vault>/.epitype/memory_fts.sqlite3`，Git 會忽略它。`query` 與 `recall` 不會建立缺少的索引：回傳非零 JSON `no-index` 錯誤前，會先把既有的舊 `.cairn` 索引原子搬到現行位置；若搬移受阻，則直接讀取舊快照並回傳 `index_migration_pending: true`。已有有效索引但沒有命中時，仍維持原本的零結果 payload；既有索引若已過期，仍會做增量更新，該次回覆會帶 `index_updated: true`。若新舊目錄同時存在，Epitype 會採用現行索引，並提示舊目錄可在驗證後刪除。

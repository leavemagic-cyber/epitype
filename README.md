...
> Release status: pre-1.0. The repository is usable for synthetic evaluation, including default read-time supersession filtering; the release exam is not complete.
...
2. **Decision supersession contract.** Decision cards carry a stable key, status, effective time, and decision source. The lint gate rejects multiple active cards for one key and warns when a key has none. `query` and `recall` exclude `status=superseded` cards by default, retain their indexed provenance, and return a current-decision guidance line when the replacement is not already in the results. `--include-superseded` deliberately restores both versions for archaeology, while prompt-time hook recall inherits the safe default.
...
| The write side knows a decision is obsolete, but the read side can still return it | Structured decision status, default query/recall exclusion, retained provenance, and an explicit archaeology override | `python epitype/memsearch.py --selftest`<br>`python adapters/claude/recall_hook.py --selftest`<br>`python epitype/decision_lint.py --selftest` |
...
Create a vault's local FTS index explicitly, then query or recall from it:

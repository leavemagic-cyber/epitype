...
- The nightly host sync (dream section 14) still applies to a host whose only refusals are size caps, so a rules block over the cap no longer holds back the other host or either index for the night. A host refused for any other reason (broken markers, a block holding the user's own text) is left byte for byte untouched overnight, because unrecognised text elsewhere in that file may be the user's too. A block over its cap is still checked for broken markers and foreign text, so a cap refusal cannot hide either. Each refusal is counted, reported as a section error (the next session start says the night was not clean) and given a next-step line. Before, a refusal skipped the whole sync and the packet said there was nothing to do.

...
- 續行程序另增 1,055 UTF-8 bytes，連同提問程序共 2,035 bytes；預算不足先完整保留原提問程序。共享回歸增至十一項，補正常停點與原生 App 分階段驗收協定；不宣稱通用語意判斷已被機械化。

- 在現有 UserPromptSubmit 與 SessionStart（開場／恢復／壓縮後）路徑加入共用的提問前查證程序；不依賴使用者問句命中記憶卡。區分已驗能力、具體開發途徑與待驗範圍，保留偏好、授權及假設設計。
- 程序與喚回共用輸出上限；每次送達增加 980 UTF-8 bytes，無額外模型呼叫。新增八項離線回歸與合成驗收流程；這是生成前指引，不是任意前提的語意硬攔截，也不代表 App 全面驗收或改善幅度已證明。

...
兩個宿主共用同一套卡片、同一套判斷，所以「裝好了」看起來是對稱的。實際去翻閘門自己的帳：
`_GATE_LOG.jsonl` 裡 40 列 `write_block` **全部來自 Claude，Codex 零列**。不是 Codex 的掛鉤沒跑
（同一份帳裡它有 114 列指令檢查、真庫還有 130 個它的已讀紀錄），是**它寫檔的那條路從來沒被看見過**。

原因是寫檔內容檢查認的是工具名稱，而這台機器的 Codex 根本沒有 `apply_patch` 這種工具：它把
`*** Begin Patch … *** End Patch` 夾在 `exec` 的文字裡送出，進到 `PreToolUse` 時工具名已經變成
`Bash`。所以把 Codex 的工具名補進名單，一行效果都不會有。

- **改認封套，不認工具名。** 新模組 `epitype/patch_envelope.py` 把封套當**純文字**拆開——不執行、
  不解析語言、不猜包著它的那段程式在做什麼。任何呼叫的輸入文字帶封套，就逐個目標檔判定。
- **只比對新增行。** `docs/FAILURE_MODES.md` §11 當初把 diff 型工具排除在外是對的：一份 diff 的
  上下文行與刪除行會命中那次寫入根本沒新增的樣式，那是誤擋。這一版沒有推翻那個理由，是讓它
  不再適用——交給規則 A 的只有 `+` 行。實測：同一句禁用語只出現在刪除行時不擋，出現在新增行時擋。
- **判不了就標未知，不牽連其他檔。** `Add File` 的新增行就是整個檔案，卡片契約照判；`Update File`
  無法從補丁還原寫入後的樣子，所以不判——與「`Edit` 的 `old_string` 不在檔案裡就不判」同一個作風。
  路徑解析不出、內容超過上限、封套壞掉：該檔標未知，同一批的其他檔照判。
- **已知限制照說**：判定只看封套文字、不猜意圖，所以 `grep` 把一份 patch 印出來也會被判；
  單次呼叫超過 40 個檔的部分不解析。兩條都寫進 §11。

### 「我讀過那個檔」的證據要分強弱

以前的已讀紀錄是從工具輸入的**全文**抓檔名，於是在指令裡提一句 `Write-Output 'ghost.py'` 就足以
讓「我讀過 ghost.py」通過檢查。現在分兩種：**強**＝結構化路徑欄位指名的目標，**弱**＝自由文字掃到的
檔名；兩種都留著供稽核，但回合閘只收強的。2026-09-22 之前寫下的紀錄沒有標記，一律當弱——分不出
來就不替自己背書。

**邊界照說**：動手閘看得到呼叫、看不到工具**結果**，所以「強」的意思是「確實對這個目標提出過讀取」，
不是「讀成功」。而且當一整場沒有任何指名讀取時，既有規則仍然不擋（分不出是閘沒裝還是真的沒動工具）。
所以這一版修好的是「弱證據不再冒充強證據」，不是「假宣稱一律抓得到」。

### 其他

- **掛鉤事件清單改成單一來源**：`hook_trust` 直接取 `install/graft.py::EVENTS`。兩份各寫一份的結果是
  安裝器裝六個、檢查器只認五個，`SubagentStop` 被報成多餘。
- **接住 Codex 的實際對話格式**：`custom_tool_call` 的 `input` 原文保留（不解析 JS 去猜 `cmd` 的值），
  交接摘要除了 `command` 也收 `cmd`。以前這兩處讓 Codex 的指令在交接與壓縮地圖裡整段消失。
- **自測不再借用使用者的暫存目錄**：`--jobs` 的說明一直寫著「每支用自己的暫存目錄」，但沒有實作。
  代價是併行跑時互相踩到同一份清掃時間戳而假敗（8 次跑紅 3 次），以及每跑一次在使用者的 `%TEMP%`
  留下數十個目錄（當天累積 2,302 項）。現在那句話是真的：修後連跑 5 次皆 79/79，暫存目錄零增長。

## v1.6.2 (2026-09-22)

> PyPI 上沒有 1.6.0 與 1.6.1：兩次都被發布閘擋下，兩次都是同一個毛病——本版新增的回歸測試
> 裡有三項在**賭機器夠慢**（用 0.05 s 的真預算賭 300 張卡掃不完、用 0.1 s 賭 300 張卡暖不完），
> 本機（Windows）綠、CI 的機器快到做得完就紅。閘擋對了，那正是它存在的理由；該檢討的是我
> 一次只修一項、沒有把同一類找乾淨。三項全部改用假時鐘讓期限必然在工作途中過去，並實證走
> 到的是「做到一半逾時」那條路而不是「期限本來就過了」。第三次改以閘自己的方式（`--jobs 4`
> 並行）在本機連跑三次確認才推。已推出的 1.6.0／1.6.1 標籤不動——移動或刪除遠端標籤是破壞性
> 操作，需要 owner 明示授權。
>
> 順帶更正兩句錯的說明：暖快取被期限截斷時**不會**寫回半份快取（實測全有全無），原本的
> docstring 宣稱「讀到的照樣留著」是錯的。它不留是對的——半份清單會讓閘把沒讀過的卡當成
> 沒有守衛，那是漏擋。代價是暖不完的庫每場從零開始；實測你這台機器的治理庫（463 張卡）
> 1.02 s 暖得完，所以會收斂。

### 這一版做完的一件事：看起來都裝好了，其實沒送到

三件事各自看起來都是好的：記憶庫在那裡、專案規則寫成卡了、開場掛鉤跑完回 0。實際量下去，
三件都沒有把東西送到該去的地方——七個裝著卡的庫從來沒被整理過、一個專案的規則一條都沒到那場
Codex 面前、開場在真實記憶庫上要跑 13.9 秒而宿主 9 秒就砍。共同點是它們都不會出聲：沒有任何
一個環節會說「我沒送到」。這一版把三處各自補上「送不到就要被發現」。

### 開場的每一段現在真的有自己的時間上限

2026-09-06 的逾時事故（Codex 把 SessionStart 記成 Failed）當時的結論是「每一段都要在剩餘預算
內完成」，但實作只在段與段之間問一次「還有沒有剩」，問完就放手。2026-09-22 逐段計時抓到兩段
還是這樣。**真實記憶庫上的數字（通用庫 463 張卡＋titan 庫 384 張，取副本量測）**：

| 段 | 修前 | 修後 | 該段預算 |
|---|---|---|---|
| `card_lint.scan_vaults` | 1.03 s | 1.03 s | 1.0 s |
| `warm_guard_cache` | **9.02 s** | 1.03 s | 1.0 s |
| `_repeat_guard_notices` | **3.90 s** | 0.50 s | 0.5 s |
| 合計 | **13.94 s** | **2.56 s** | 軟預算 5.0 s |

`warm_guard_cache` 修前那個 9.02 s 不是巧合——它逐卡比的是 `HOOK_TIMEOUT_SECONDS`（9.0 s），
也就是它一路跑到**宿主砍人的門檻**才停。合成庫上同樣三段只量到 6.10 s，其中「這道守衛擋了你
幾次」那一段是 0.00 s（合成庫沒有 gate log）——真庫才照得出 3.90 s。這也是為什麼下面那一項
「自測不再看機器快慢」單獨修不夠：那支自測只是在合成庫上看到了問題的一半。

- **守衛暖快取改成收一個絕對期限**（`warm_guard_cache(..., deadline=)`），逐庫之間與一庫之內
  逐卡都會中止。以前它逐卡比的是宿主的 9 s 逾時、張數上限是 `1 << 30`，等於「想跑多久跑多久」。
  它是純優化，時間不夠時第一個讓路：`SessionStart` 用 `_segment_budget` 撥給它
  `SESSIONSTART_WARM_GUARD_BUDGET_SECONDS`（1.0 s），撥不到就整段省略。
- **截斷不降低守衛的效力**。暖到的照樣寫回快取，沒暖到的仍由每次工具呼叫既有的「逐次讀一小片」
  收斂。截斷只影響「多快暖完」，不影響最終擋不擋得住——閘本身一張卡都沒有少讀。
- **`summary_line` 不再把逾時的那一趟重掃一次**。`reports=None` 以前同時是「還沒掃」與「共用的
  那一趟逾時了」，於是開場一逾時就用預設預算再掃一整趟——講好只付一次的預算，正好在付不起的
  時候付兩次。現在「沒帶 reports」才自己掃，`None` 就是逾時，直接不出聲。
- **`scan_vaults` 期限過了不再起下一個庫**：逐卡的檢查只救得了「這一庫掃不完」，而下一庫的第一趟
  目錄走訪本身沒有上限。逾時回 `None` 的語意不變（半個庫的數字是錯的數字）。
- **「這道守衛最近擋了你幾次」那一行也收一個期限**（`SESSIONSTART_GUARD_NOTICE_BUDGET_SECONDS`，
  0.5 s）。它本來只靠 `ACTION_GUARD_MAX_CARDS_PER_VAULT` 收斂——那是**張數**上限不是**時間**上限，
  卡變大、變多或磁碟變慢時同一段照樣吃得掉預算。數次數的那一圈逾時就整段回空：只數了一半的庫
  給出的「擋了 N 次」是錯的數字，而這一行的全部內容就是那個數字；查建議詞的那一圈逾時只是中止，
  該印的行照印（建議詞是裝飾，本來就容許空的）。

### 專案的規則現在送得到專案裡的 Codex

2026-09-20 實測事故：一場 Codex 在 `C:\projects\自主賺錢方案` 跑了一整場，那個專案的規則卡一條
都沒到它面前。Codex 開場只讀兩個地方——全域 `~/.codex/AGENTS.md` 與**專案根的 `AGENTS.md`**，而
全域那一份刻意只收治理庫的規則（專案規則不該被升格成跨專案契約），專案根又沒有 `AGENTS.md`，
於是那個專案的規則沒有任何送達面。Claude Code 沒出事只是因為宿主自己會載入 cwd 專案庫的
`MEMORY.md`，Codex 沒有這個機制——而看起來兩邊都「裝好了」。

- **`epitype sync` 現在也寫專案根**：每個裝著卡的原生專案庫，把**它自己**的規則卡生成一塊寫進
  `<專案根>/AGENTS.md` 與 `<專案根>/CLAUDE.md`，索引（那個庫的 `MEMORY.md`）只寫進 `AGENTS.md`
  ——Claude Code 本來就會載入 `MEMORY.md`，再寫一份是同一段字每場付兩次。規則卡一張都沒有的庫
  不建空塊；兩塊都沒內容就連檔案都不建。
- **專案根靠對話紀錄反查，不開名冊**（owner 2026-09-21：不應該是登記制）。slug 反推不回原路
  徑，所以讀庫所在專案目錄裡最新幾份 `*.jsonl` 的 `cwd`；反查不到的庫印一行 SKIP 說清楚，不猜。
  判定與夜間夢盤點孤兒庫是同一個函式（`capture_route.project_root_of`）。
- **專案根是另一個專案根的上層目錄就整個跳過**。兩個宿主都沿路往上讀祖先目錄的 `AGENTS.md`／
  `CLAUDE.md`，所以寫進上層＝把那一庫的卡塞進底下每一個專案的每一場（實測有一個庫的專案根
  就是桌面，底下有四個裝著卡的專案）。純祖先關係判斷，沒有特例名單；跳過時印一行說是被誰擋的。
  比對的是這台機器上**每個裝著卡的原生庫**反查得出的根，不是只有這一次要寫的那幾個——保護
  不該取決於底下那個庫這次剛好有沒有內容可寫。空殼後代不擋人，因為空殼不是庫（宿主會替每個
  cwd 開空目錄，包含專案自己的子資料夾；照開過就算的話，在自己專案裡跑過一場就會讓那個專案
  的根永遠寫不進去）。**擋不到的那一半照說**：從來沒被開過、一份對話紀錄都沒有的專案反查不出
  根，它的上層目錄我們保護不了。
- **兩個以上的庫指到同一個專案根時全部跳過**，並指路去 `capture_route --audit` 歸戶。對同一個
  檔產生兩組目標＝後寫蓋前寫，而且每晚來回翻；哪一個庫才是那個專案的，產品不替使用者挑。
- **「只動兩個標記之間」現在是真的逐位元組**。`render` 替換既有區塊時原本會把區塊後面的換行整個
  吃掉（`tail.lstrip("\n")`）；全域宿主檔的區塊都在檔尾、後面沒東西，所以一直看不出來，寫進使用者
  自己的專案檔就是把他區塊後面那一行空白吃掉（實測 titan 的 `AGENTS.md` 就是這樣少一行）。改成
  `head + block + tail`，標記外面一個位元組都不碰。`--remove` 把整塊拿掉時前後空白會碰在一起，
  逐位元組不可能，改成留「兩邊原本各自帶的換行數裡較多的那一個」，前後各一個空行的常見寫法
  因此原樣不動。
- **寫法與全域宿主檔同一套**：只動標記之間、標記不成對就整個檔停手、第一次寫前留 `.epitype-bak`、
  認不出的原文先存副本再取代、超過上限拒寫（`AGENTS.md` 一樣守 Codex 的 `project_doc_max_bytes`）。
  專案根原本由私人腳本同步的舊標記認得，`--apply` 就地換成產品標記，不會長出第二塊。
- **`--remove` 拿得回來**，專案檔只拿掉區塊、其餘位元組不動；拿掉之後只剩空白時，只有「我們
  從無到有建的那一個」（動它之前沒有備份＝原本根本沒有這個檔）才刪，使用者原本就有的留著。**`epitype doctor`** 每個專案庫印一行 `PROJECT: <root> AGENTS.md=… CLAUDE.md=…`，
  反查不到專案根的印 `root=unknown`。夜間夢走的是同一條 `host_sync`，所以這件事每晚自己會跑。

### 裝著卡的記憶庫本來就受管，不必先登記

設定檔的 `vaults` 被當成名冊用了：宿主替每個專案開的原生記憶庫，只有被人手動寫進那一行的才
進夜間夢、卡片檢查與視圖。實際上這台機器有 11 個原生庫裝著卡，登記的只有 4 個——其餘 7 個
（含一個帶 9 張卡與 8 張事件卡的專案庫）從來沒被整理過，而且不會有任何徵兆：夜間報告本來就
會數「沒登記卻裝著卡」的庫，卻把「專案資料夾還在」的那些直接跳過，正在用的專案反而永遠不
會被列出來。

- **受管＝設定登記的 ＋ 掃描到裝著卡的原生庫**。判準只有一個（`capture_route`）：安裝器、夜間
  夢、落點與 doctor 都問同一份，空殼目錄一律不算庫。掃描只在 CLI 與夜間這一側做；喚回維持以
  cwd 解析，每一則提問不會為了這件事多付一次全機掃描。
- **doctor 會把掃描到的庫一起報出來**，不再讓「不在 config」被讀成「沒人管」。
- **主記憶整形改認每一庫自己的手寫段**。「`MEMORY.md` 要保持精簡索引」是所有庫共通的規則，
  所以整形每一庫都跑；但原本的保護是四個寫死的段名，而那四個其實只是治理庫自己的段名。別的
  庫用自己的段名（例如「## 專案執行約束」）會在第一晚整段被搬進 `_drafts/index_pruned`——而
  專案庫的 `MEMORY.md` 正是宿主每一場自動載入的那一份。現在每一庫第一次被整形時，先把自己
  當下的段名記進 `<vault>/.epitype/index_sections.json`，**那一次一行都不搬**；之後只有清單以
  外、事後才長出來的段落裡、且目錄已經承載的卡片連結才會被搬走。清單讀不出來就當成第一次：
  不搬、重記、在報告裡說一行。

### 開場自測不再看機器快慢忽好忽壞

開場（`sessionstart`）自測有三項把「某一行有沒有出現」當成硬斷言，而那幾行受開場自用預算控制：
機器一忙，前面的卡片檢查就把預算吃完，後面的月亮夢通知、卡片檢查錨、夢完成通知整段被省略，
`_handle` 直接回空——空輸出是宿主完全接受的（宿主砍 hook 只有逾時與吐壞 JSON 兩個理由），自測
卻判成失敗。實測同一份程式在乾淨機器上全過、在忙碌機器上三項假敗（master 18 跑 6 敗，全在慢輪）。
三項都改成釘真正的不變量：Codex 事件「空輸出或恰好兩個鍵、絕不吐壞 JSON、絕不退回原始索引回音」；
§35「有輸出時才要求錨，裁定永不進開場」；夢完成通知「跨兩場至多宣告一次、內容對、絕不重印」。
八個燒 CPU 的行程滿載時連跑 12 次全過。

### 設定檔位置推錯家目錄不再靜默少管

裝著卡的原生庫由設定檔所在位置往上推家目錄再掃描；家目錄底下沒有 `.claude` 也沒有 `.codex` 時
（多半是 `EPITYPE_CONFIG` 指到家目錄以外）掃描一無所獲，以前只會靜默地少管幾個庫。現在 CLI 與
夜間解析受管庫時會往 stderr 說一行，正常安裝（設定檔在 `<home>/.epitype/` 底下）不觸發。

## v1.5.0 (2026-09-21)

### 這一版做完的一件事：規則要擋得準，而且不准為了擋而浪費

上一版讓卡片從「被讀到」變成「擋得住」。這一版問的是下一個問題：**擋得對嗎、值得嗎**。
七天的真實使用量出來的答案是不夠好——一道守衛擋下 147 次真實指令，其中 136 次（93%）是誤擋；
回合被擋 105 次裡有 19 次只是用詞與篇幅，而宿主是先把字顯示給使用者、再跑檢查，那 19 次擋下來
一個字都收不回去，只是逼出第二輪。為了省 token 設的規則自己在燒 token。

- **`on_hit: note`**：規則照樣比對、照樣進帳，但不擋回合；違規改成附在下一則提問上的一行提醒。
  用詞與篇幅類的規則改用它。擋下與提醒在夜間報告裡分開計數。
- **重貼整段不再擋**：觸發的那一刻，重複的內容早就在使用者眼前了。「不要整段重貼」改放進第一次
  擋下的理由裡——那是唯一來得及的時機。
- **`epitype.compliance --samples`**：拿最近幾天的真實對話，列出每條規則實際命中的句子。作者自己
  想的例句只測得到作者想得到的情況；這支指令第一次跑就照出多條規則大半在誤擋，也照出夜間重放與
  現場檢查判讀不一致（證據是引文時，現場放行、重放卻算成漏擋）。兩邊現在用同一個判讀。
- **舊狀態卡會自己說有多舊**：專案卡記的是某個時間點的狀態，但喚回只給一行摘要，三個月前的狀態
  跟昨天寫的長得一樣。超過 30 天的專案卡，那一行前面會加上「⚠ N 天前的狀態，引用前先查現況」。
  裁定與教訓不標——它們不會因為放久就失效，而每一行都標等於沒標。
- **掛鉤登記落後會被發現**：程式與登記是兩半，程式改了立刻生效，登記要跑安裝才更新。直接跑原始碼
  的機器永遠不經過「升級後重裝」，新增一個時機點之後登記落後沒有任何徵兆。夜間檢查現在會比對，
  落後就在隔天開場說一行。
- **段落指稱不算待辦**：「§9待決」講的是文件第 9 節，不是一件沒做完的事。

### 陌生人裝完不再是空的

- **`epitype starter`**：五張通用規則卡，一行指令放進記憶庫（不覆蓋你已有的檔案，`--remove` 只收回
  你沒動過的）。不准一次加入整棵樹（Bash 與 PowerShell 各一張）、說做完要附證據、不准說「應該修好了」、
  客套開場白只提醒。每張都帶「一定要擋」與「一定不能擋」兩種例句，寫卡當下用閘門自己的程式碼跑過。
- **顯示語言可切換**：所有擋下訊息有英文與繁體中文兩套。新安裝依系統語系決定，`~/.epitype/config.json`
  的 `language` 或 `EPITYPE_LANG` 可改。規則樣式、欄位名與比對內容不受影響。
- **首頁補上證明**：三十秒示範、真的被擋下來的原樣輸出、健康檢查的實際輸出、資料去哪裡（不連網、
  不回傳、擋下紀錄不含訊息內容）、與同類工具的定位對照。

### 規則本身的修正（依真實被擋的句子重放）

「說做完要附證據」認得提交編號與逐行核對；「待辦清單」要真的在列清單才觸發；「更正要帶重查」不再把
名詞用法與單純附和算進去；「從沒…」只認講自己過去沒做過某件工作；數字規則不再把日期當成比例；
「不准放寬門檻」不再把否定句算成做了；子代理回報那條收掉，由「派工之後我自己有沒有動手」的事實檢查取代。
一週試行的「報告要列讀了哪些檔」提前收掉——同一件事已經有硬檢查在做，它每天只是要求多寫一行。

### 其他

- 測速工具改量記憶庫的暫存副本：它原本每跑一次就往正式紀錄灌十幾列假的擋下，正式紀錄有 38% 是這樣來的。
- 動作守衛的 heredoc 條件收窄成「連續兩個反斜線」——只有那一種真的會被吃掉一層。
...
- Recall pins a decision card with its description (what was decided, in understood words) instead of the owner's verbatim quote, and the Stop and write gates name the ruling the same way when they block. The quote stays on the card as evidence, one level down; a fragment such as 「B」（Q7） injected on its own told the model nothing.
- `epitype sync` replaces text inside its own blocks that it does not recognise as its own, instead of refusing the block, and first appends the original to `<host file>.epitype-replaced.md` with who replaced it (manual sync, installer, nightly dream, or the Claude session-end hook via `--actor`) and when. A refused block had meant rules that never reached the agent; overwriting silently had meant lost text. The file sits next to the host file so uninstall (which deletes `~/.epitype`) keeps it, and uninstall names it. Broken marker pairs still leave the whole file untouched.
- The nightly host sync (dream section 14) still applies when one block is refused, so a rules block over the cap no longer holds back the other host or either index for the night. Refusals are counted, reported as section errors and given a next-step line; replacements are listed with who made them and where the original went. Before, a refusal skipped the whole sync and the packet said there was nothing to do.
- Dream section 11 compares `core_files` against the governance vault's rules only, as `epitype sync` does, and measures them per host. With a project vault that has rule cards registered, it reported drift every night.
- `epitype sync` refuses a rules block larger than the configured `core_cap_bytes`. Only core-gen honoured the cap, while the nightly sync that writes what agents actually read ignored it.
...
2026-09-16 一整天的實測揭開了一個前提錯誤：**送達不等於執行**。同一個 heredoc 吃反斜線的錯誤在一場對話裡犯了四次，三張相關卡片都在庫裡、都喚得回來、模型也讀到了，行為一次都沒改變。同一天量到契約的情境層觸發只有 4.9% 真的被讀（81 場該觸發、4 場去讀），改成全部常駐之後，59 條規則在眼前照樣被違反六次。「常駐 vs 觸發」這條軸線兩端都只是送達；真正的軸線是**送達 vs 攔截**。

所以卡片現在有三種綁得住的形式，全部字面比對、不讀意圖：

- **`forbidden`**（欄位早就存在，從沒用在行為上）：回合結束前比對模型說出口的話，命中就擋回去重寫。實查當時：庫裡 400 張卡只有 11 張帶 `forbidden`，而那 11 張全是產品自己的設計決策，**關於行為的 0 張**。
- **`require_when` + `require_text`**：給「做了才准說」那一類規則。「宣稱完成前先驗」沒辦法檢查，因為有沒有驗發生在腦袋裡；改成「說完成的同一則訊息要寫出查了什麼」，漏掉就變成看得見的。跳過的成本也變了——要嘛照做，要嘛寫一個假的證據，那是說謊不是省略。
- **`guard_tool` + `guard_all_of`**：一次工具呼叫的文字同時含有卡片指名的每一個字面片段就拒絕。這一項 2026-09-09 曾被 owner 移除（FAILURE_MODES §34），2026-09-16 解除：當初移除的前提是「交給宿主原生規則」，實測不成立——Claude 的 Bash 權限樣式位置比對、沒有 AND 運算子，九類危險四類搬得過去、五類根本寫不出來。回來的只有字面連言，語意判斷與不可逆動作仍然是宿主的事。**同一個指令換個等價寫法就過得去，這是設計上接受的：它防重犯，不防人。**

### 夜裡自己檢討、自己改進

閘在使用者等回話的當下跑，有期限、有每次呼叫的讀卡上限、出錯一律放行——它注定會漏，而且漏的時候不出聲。`epitype/compliance.py` 在夜裡沒有這些限制：同一批卡、同一套比對，重放整天的對話紀錄，再跟閘自己的稽核帳對帳，對得上就是當場擋下了，對不上就是漏了。刻意重用閘自己的讀卡與遮罩程式，重放若用不同判讀，對不上代表檢查器寫錯，那種「發現」比沒有更糟。

三件事不算漏擋，而且對帳說得出理由：同一則訊息重複出現（閘本來就只擋一次）、回合中段的訊息（Stop 閘從來看不到，另外歸類）、以及同一則訊息已被別張卡擋下（閘一次只報一個理由，所以兩道閘的稽核列現在帶訊息指紋，內容本身仍然一個字都不記）。

夢做完自己改，不產生任何需要人讀的東西：把命中併進健康度、照「近期真的攔到東西」重排兩道閘的讀卡順序、把「擋了等於沒擋」的那一則訊息自動放行。

### 規則終於到得了代理面前

以前 Epitype 只到「產生得出規則塊」為止——代理讀的是 `~/.claude/CLAUDE.md` 與 `~/.codex/AGENTS.md`，不是產生出來的中間檔。`epitype sync` 補上這一段：只動兩個標記之間、標記重複或顛倒一律拒絕不猜、認不出是自己寫的內容就停手、先備份再原子寫入、每塊有位元組上限。安裝時做一次，夢每晚檢查漂移並自己補上，`epitype sync --remove` 與 `graft uninstall` 拿得回來。

### 這一版的缺陷是四輪對抗審查抓出來的

作者自己的測試全綠（54/54）時，四份獨立審查一致判「不可發布」，其中兩項是「產品寫進使用者的全域指令檔卻拿不回來」。列出來的都是實跑重現過的：

- 自動放行原本是把命中字串加進清單，而**字面樣式的命中字串就是樣式本身**——放行一串字等於把整條規則關掉 90 天。改成只認那一則訊息的指紋，這條路徑再也不可能關掉一條規則。
- 相似度原本比整則訊息，四百字的回覆就算照規則整句重寫也達 0.98。改成只比命中所在的那一句。
- **自動降級整個移除。** 一條 6 次命中、6 次全部擋下、0 漏擋的規則，因為命中率 0.12 被關 14 天，連 heredoc 那張安全守衛卡都一起關，而且沒有任何地方會提。命中率分不出「規則太寬」與「我一直犯這條」，後者正是好規則的樣子——判準錯了就不是調門檻的問題。數字照記，不再有任何東西因為它停止攔截。
- 工具回傳被當成回合邊界，分母虛高約 6 倍（一份真實紀錄 300 個 user 列裡有 279 個是工具回傳）；對話檔只讀 24 個而當天有 101 個，報告卻只寫「24」。兩個都修了，少讀幾個現在會報出來。
- 引號遮罩是自助式的：把整段主張包進「」、直引號、反引號或 `>` 引用行，四種寫法實測全部靜音。加了兩道上限（遮罩佔整則過半就整則不遮、單一引用區段超過 120 字不遮），**原本那四種寫法現在實測全部擋得住**（第五輪對抗審查逐種實跑；擋下來的主要是佔比那一道）。**剩下的殘留修不掉**：一句短的違規話加引號，跟一次正當的引用在字面上完全一樣。所以第三道不是再收窄，是留痕——遮罩放過的豁免寫一列 `stop_masked` 稽核（卡名、片段、訊息指紋，內容不記），夜間數成 `quoted_exemptions`。粒度是一則訊息對一張卡一列。分不出來的東西至少數得出來。
- 每回合的讀卡上限原本連「執行」一起砍，500 張武裝卡只有 12 張生效，而且要先被放行過一次才拿得到優先權——死結。上限改成只管「找」。
- 照文件寫的 `python install/graft.py install` 形式，`epitype` 匯不進來，**整個宿主檔同步被安靜跳過而安裝照樣回報成功**。
- 隔離驗收兩次刪掉真實機器上服役中的夜間排程：`--home` 隔離不到系統排程，而拿家目錄去比也沒用（沙盒本來就會換掉 `HOME`）。改成只有「這個家目錄原本真的是 nightly」才反註冊。

### 其他

- `card_lint` 認得會靜默失效的卡：欄位被包進下一層（看起來武裝、實際什麼都不擋）、守衛條件太寬到等於停用整類工具、半條 require、樣式編不起來、工具名打錯。行為型的卡必須二選一：寫出擋得住的欄位，或寫出 `unenforceable:` 與理由。
- 樣式編不起來時閘退回逐字比對，那張卡不再完全失效。**編得起來但意思跑掉的（`完成(暫定)` 是合法正則，去找「完成暫定」）抓不到**——試過的判準把 62 條真實樣式裡 33 條正確的也標成可疑，沒有裝上去。
- README、中文 README、ARCHITECTURE 三處「卡片擋不住任何一次工具呼叫」已不是事實，改成與實際一致；README 原本說夢只寫 vault 裡面，實際上它每晚無人看管地改寫使用者的全域指令檔，也改成講清楚。
- FAILURE_MODES §34 標成「部分被取代」，不是改寫成跟現在一致。
- tests：新增 `tests/action_guard_regression.py`，run_all 53 → 54。

...
> v1.6.0 的標籤推出去了，但發布閘擋下來，所以 PyPI 上沒有 1.6.0——本版新增的那支回歸測試
> 裡有兩項在賭「機器夠慢」（用 0.05 s 的真預算去賭 300 張卡掃不完），本機綠、CI 的機器快到
> 掃得完就紅。閘擋對了：那正是它存在的理由。兩項改用假時鐘讓期限必然在掃描途中過去，結果
> 與機器快慢無關。已推出的標籤不動（移動或刪除遠端標籤是破壞性操作），改以 1.6.1 發布。

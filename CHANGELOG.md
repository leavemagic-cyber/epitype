...
- The nightly host sync (dream section 14) still applies when one block is refused, so a rules block over the cap no longer holds back the other host or either index for the night; each refusal is counted, reported as a section error (the next session start says the night was not clean) and given a next-step line. Before, a refusal skipped the whole sync and the packet said there was nothing to do.

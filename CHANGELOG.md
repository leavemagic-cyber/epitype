...
- 引號遮罩是自助式的：把整段主張包進「」、直引號、反引號或 `>` 引用行，四種寫法實測全部靜音。加了兩道上限（遮罩佔整則過半就整則不遮、單一引用區段超過 120 字不遮），`>` 引用行與整段包起來的那兩種因此擋得住了。**剩下的殘留修不掉**：一句短的違規話加引號，跟一次正當的引用在字面上完全一樣。所以第三道不是再收窄，是留痕——遮罩放過的豁免寫一列 `stop_masked` 稽核（卡名、片段、訊息指紋，內容不記），夜間數成 `quoted_exemptions`。粒度是一則訊息對一張卡一列。分不出來的東西至少數得出來。
